...
class NowPlayStanciyaViewController: UIViewController {
    ...
    var currentStation: RadioStanciya!
    var currentTrack: Pecnya!
    ...
    func setupVolumeSlider() {
        ...
        mpVolumeSlider.tintColor = UIColor(hex: "#7ECE5C")
    ...
    func load(station: RadioStanciya?, track: Pecnya?, isNewStation: Bool = true) {
    ...
    func updateTrackMetadata(with track: Pecnya?) {
    ...
    func updateTrackArtwork(with track: Pecnya?) {
        guard let track = track else { return }
    ...
    func playbackStateDidChange(_ playbackState: FMRadioPlaybackState, animate: Bool) {
    ...
    func playerStateDidChange(_ state: FRadioPlayerState, animate: Bool) {
        ...
        switch state {
        case .loading:
            message = "Загрузка Станции ..."
        case .urlNotSet:
            message = "NowPlayStanciyaViewController"
    ...
    func createNowPlayingAnimation() {
        ...
        // Setup ImageView
        nowPlayingImageView = UIImageView(image: UIImage(named: "equalizer-3"))
        nowPlayingImageView.autoresizingMask = []
        nowPlayingImageView.contentMode = UIView.ContentMode.center
        
        // Create Animation
        nowPlayingImageView.animationImages = AnimationCustomFrames.createFrames()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard segue.identifier == "InfoDetail", let infoController = segue.destination as? InfoStanciyaViewController else { return }
    ...
    @IBAction func shareButtonPressed(_ sender: UIButton) {
        let songToShare = "I'm listening to \(currentTrack.title) on \(currentStation.name) via Best Russian Radio"
